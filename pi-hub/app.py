...
from mappings import value_types, devices
...
async def get_or_create_device_client(device_id, device_clients):
    ...
    device_key = compute_derived_symmetric_key(device_id).decode("ascii")

    # Register the device with IoT Central. If the device doesn't exist, it will be created
    registration_result = await register_device(device_id, device_key)
...
def build_telemetry(value_type, value):
    ...
    # Get the telemetry value by converting the string to a float
...
async def main():
    ...
    listeners = asyncio.gather(message_worker(queue))
